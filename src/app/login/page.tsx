...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (!GOOGLE_CLIENT_ID) {
      ...
      setError("Google login is not configured yet. Please use username/email + password.");
      return;
    }
    ...
    const renderGoogleButton = () => {
      ...
      if (!window.google?.accounts?.id) {
        setGoogleLoading(false);
        setError("Google sign-in could not load. Please refresh the page or use username/email + password.");
        return;
      }
      try {
        ...
        <div className="bg-white rounded-3xl p-8 shadow-xl shadow-pink-100/50 border border-pink-100">
          ...
          <div className="mb-4 w-full rounded-xl border border-gray-200 bg-white p-2 shadow-sm">
            ...
            {!googleLoading && !googleRendered && (
              <div className="py-3 text-center text-sm font-bold text-gray-500">Sign in with Google</div>
            )}
          </div>
