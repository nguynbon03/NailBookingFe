...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.location.href = user.role === "STAFF" ? "/staff" : ["ADMIN", "MANAGER"].includes(user.role) ? "/admin" : "/";
    }
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md mx-4">
        <div className="bg-white rounded-3xl p-8 shadow-xl shadow-pink-100/50 border border-pink-100">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-500 text-sm mt-1">Sign in with admin / admin123 to manage bookings</p>
          </div>
