...
import { Menu, X, Phone, CalendarDays, Sparkles } from "lucide-react";
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="px-4 py-2 rounded-full text-sm font-medium text-gray-600 hover:text-pink-600 hover:bg-pink-50 transition-all"
              >
                {link.label}
              </Link>
            ))}
        ...
        <div className="lg:hidden bg-white/95 backdrop-blur-xl border-t border-pink-100 px-4 py-6 space-y-2 shadow-xl">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="block px-4 py-3 rounded-xl text-gray-700 hover:text-pink-600 hover:bg-pink-50 font-medium transition-all"
