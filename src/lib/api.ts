...
export const api = {
  ...
  notifications: {
    list: (audience?: string, take = 50, category?: string) =>
      fetchAPI(`/api/notifications?take=${take}${audience ? `&audience=${audience}` : ""}${category ? `&category=${category}` : ""}`),
    markAll: (audience: string, category?: string) =>
      fetchAPI("/api/notifications", { method: "PUT", body: JSON.stringify({ audience, read: true, category }) }),
    markOne: (id: string, read = true, audience = "admin", category?: string) =>
      fetchAPI("/api/notifications", { method: "PUT", body: JSON.stringify({ id, read, audience, category }) }),
    deleteOne: (id: string, audience = "admin", category?: string) =>
      fetchAPI("/api/notifications", { method: "DELETE", body: JSON.stringify({ id, audience, category }) }),
    deleteMany: (ids: string[], audience = "admin", category?: string) =>
      fetchAPI("/api/notifications", { method: "DELETE", body: JSON.stringify({ ids, audience, category }) }),
    deleteAll: (audience = "admin", category?: string) =>
      fetchAPI("/api/notifications", { method: "DELETE", body: JSON.stringify({ audience, category, readOnly: false }) }),
    action: (notificationId: string, action: string, managerNote?: string) =>
      fetchAPI("/api/notifications/action", { method: "POST", body: JSON.stringify({ notificationId, action, managerNote }) }),
  },
  ...
  payments: {
    ...
    claimTransfer: (token: string, note?: string) =>
      fetchAPI("/api/payments/transfer/claim", { method: "POST", body: JSON.stringify({ token, note }) }),
  },
  ...
  staff: {
    ...
    action: (id: string, action: string, cancellationReason?: string | null) =>
  ...
  admin: {
    ...
    updateBookingStatus: (id: string, status: string, staffId?: string | null, cancellationReason?: string | null) =>
      fetchAPI("/api/bookings", { method: "PUT", body: JSON.stringify({ id, status, staffId, cancellationReason }) }),
    ...
    reviewLeave: (id: string, status: string, managerNote?: string) =>
      fetchAPI("/api/staff/leave", { method: "PUT", body: JSON.stringify({ id, status, managerNote }) }),
    ...
    importBankStatement: (csv: string) =>
      fetchAPI("/api/admin/bank-statements", { method: "POST", body: JSON.stringify({ csv, source: "admin_upload" }) }),
